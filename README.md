...
[![Status](https://img.shields.io/badge/状态-BETA%20v0.1.0-yellow?style=for-the-badge)](https://github.com/BH2VOQ/PS3Eye-VirtualCam/releases)
...
- 🔋 **无消费者自动停止**：没有 App 在用时摄像头自动断电省电，有 App 打开时自动恢复
...
│  (OV534)    │   USB 抓帧    │  (本项目主程序)   │  NV12 帧 │ Camera 扩展       │
└─────────────┘               └──────────────────┘          │ (系统级虚拟摄像头) │
       640x480@60fps                 │  ▲                   └───────────────────┘
                                     │  │ 读 /tmp/ps3eye_consumer.txt（每秒）
                                     ▼  │                          │
                              ┌──────────────────┐                  ▼
                              │ consumer_watch   │      ┌─────────────────────┐
                              │ (独立检测进程)     │      │ Photo Booth / Zoom  │
                              │ AVFoundation 查询 │      │ OBS / vtuber 等 App  │
                              └──────────────────┘      └─────────────────────┘
```

**三层设计（为什么这样做）：**

1. **ps3eye-feed（主程序）**：用 libusb 从 PS3 Eye 抓帧（NV12 640x480），通过 CMIO（CoreMediaIO）往 OBS 虚拟摄像头的 sink 流喂帧。OBS 扩展负责把 sink 流转发为系统级 source 流。
2. **consumer_watch（独立检测进程）**：每秒用 AVFoundation 查询 OBS 虚拟摄像头是否被某个 App 使用（`isInUseByAnotherApplication`），把结果写进 `/tmp/ps3eye_consumer.txt`。
   > ⚠️ 检测必须在独立进程——实测在 feed 进程内直接调 AVFoundation 会与 libusb 并发冲突导致崩溃（pthread_mutex_lock 断言）。
3. **自动启停**：feed 每秒读检测结果——无消费者 → `cam->stop()`（LED 灭、省电）；有消费者 → `cam->start()` 自动恢复抓帧。
...
| 路径 | 说明 |
|---|---|
| `src/ps3eye-feed.mm` | 主程序：libusb 抓帧 → CMIO 喂帧 OBS 虚拟摄像头 |
| `src/consumer_watch.mm` | 独立消费者检测进程（AVFoundation） |
...
| 摄像头 LED 不亮？ | 正常——无消费者时自动省电熄灭；打开 App 选中后会自动亮起 |
...
- 当前版本：**v0.1.0-beta**（功能可用，画面待最终人工验收）
...
> ⚠️ **BETA — v0.1.0-beta**
> Feature-complete but the picture is not yet finally human-verified. Issues and feedback welcome.
...
- 🔋 **Auto power-off when unused**: camera sleeps when no app consumes it, wakes automatically on demand
...
┌─────────────┐    libusb     ┌──────────────────┐   CMIO   ┌───────────────────┐
│  PS3 Eye    │ ────────────▶ │  ps3eye-feed     │ ───────▶ │ OBS Virtual       │
│  (OV534)    │   USB frames  │  (this project)  │  NV12    │ Camera extension  │
└─────────────┘               └──────────────────┘          └───────────────────┘
       640x480@60fps                 │  ▲                          │
                                     │  │ reads /tmp/ps3eye_consumer.txt
                                     ▼  │                          ▼
                              ┌──────────────────┐      ┌─────────────────────┐
                              │ consumer_watch   │      │ Photo Booth / Zoom  │
                              │ (separate procs) │      │ OBS / vtuber apps   │
                              └──────────────────┘      └─────────────────────┘
```

**Three-layer design:**

1. **ps3eye-feed**: grabs NV12 640x480 frames from the camera via libusb and pushes them into the OBS virtual camera's sink stream through CMIO.
2. **consumer_watch**: polls `isInUseByAnotherApplication` via AVFoundation every second, writes the result to `/tmp/ps3eye_consumer.txt`. It runs as a **separate process** because calling AVFoundation inside the feed process crashes against libusb (pthread_mutex_lock assertion — verified).
3. **Auto power management**: feed reads the flag every second — no consumer → `cam->stop()` (LED off, power saving); consumer present → `cam->start()` resumes.
...
| Path | Description |
|---|---|
| `src/ps3eye-feed.mm` | Main feeder: libusb capture → CMIO frames into OBS virtual camera |
| `src/consumer_watch.mm` | Separate consumer-detection process (AVFoundation) |
...
| LED not on? | Normal — auto power-off when unused; it lights up when an app selects the camera |
...
- Current version: **v0.1.0-beta** (feature-complete, final human picture verification pending)
