...
case "$METHOD" in
  ...
  POST)    curl -s -X POST -H "Authorization: Bearer $TOKEN" -H "Accept: application/vnd.github+json" -H "Content-Type: application/json" -d "$BODY" "https://api.github.com$PATH2" ;;
  PATCH)   curl -s -X PATCH -H "Authorization: Bearer $TOKEN" -H "Accept: application/vnd.github+json" -H "Content-Type: application/json" -d "$BODY" "https://api.github.com$PATH2" ;;
