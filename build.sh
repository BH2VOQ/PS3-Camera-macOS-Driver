...
# 产物：bin/ps3eye-feed（喂帧主程序）、bin/consumer_watch（消费者检测进程）
set -e
cd "$(dirname "$0")"

echo "🔧 构建 PS3Eye-VirtualCam (arm64)..."

# 1. consumer_watch — 独立消费者检测进程
clang++ -std=gnu++14 -stdlib=libc++ -fobjc-arc -O2 -arch arm64 \
  -framework Foundation -framework AVFoundation \
  src/consumer_watch.mm -o bin/consumer_watch
echo "  ✅ bin/consumer_watch"

# 2. ps3eye-feed — 抓帧喂 OBS Virtual Camera（静态链接 libusb）
